...
=item B<ASCII>=I<true|false>

Use ASCII characters in place of UNICODE Box-drawing characters.

=item B<Color Decorations>=I<< <char> >>

Color for the decorations.
May be one of black, red, green, yellow, blue, magenta, cyan, white, bright black, bright red, bright green, bright yellow, bright blue, bright magenta, bright cyan, bright white.

=item B<Color Title>=I<< <char> >>

Color for the title.

=item B<Color Label Core>=I<< <char> >>

Color for the labels in main section.

=item B<Color Text Core>=I<< <char> >>

Color for the text in main section.

=item B<Color Label Extra>=I<< <char> >>

Color for labels in extended sections.

=item B<Color Text Extra>=I<< <char> >>

Color for text in extended sections.

=item B<Color Notes>=I<< <char> >>

Color for notes.

=item B<Color Control On>=I<< <char> >>

Color for control on.

=item B<Color Control Off>=I<< <char> >>

Color for control off.
...
=item B<Engine>=I<< <RESIDFP|SIDLITE|EXSID|USBSID|RESID|HARDSID|NONE> >>

Set the SID emulation engine. Default is RESIDFP if available.
...
emulation.  Ranges from 0.0 (dark) to 1.0 (light), the default
value is 0.5.
...
=item Stian Skjelstad

SID status display.

=item Michael Hartmann
...
=item Craig Chamberlain and Harry Bratt

Authors of COMPUTE!'s Sidplayer.

=item David Reid

Author of miniaudio.

=item Victor Zverovich

Author of fmt.

=item Steffen Schümann

Author of filesystem.
...
=item Compute's Gazette Sid Collection (CGSG): L<https://www.c64music.co.uk/>
...
=item Copyright (C) 2010-2026 Leandro Nini
